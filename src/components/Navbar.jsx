  ...
  return (
    <nav>
        <div className="container nav__container">
            <Link to="/" className='nav__logo'>Vote<span className="red-x">X</span></Link>
            <div>
                {showNav && <menu>
                 ...
                 <button className="theme__toggle-btn" onClick={changeThemeHandler}>{darkTheme ?<IoMdSunny size={21}/>:<IoIosMoon size={21}/>}</button>
                 <button className="nav__toggle-btn" onClick={()=>setShowNav(!showNav)}>{showNav ? <AiOutlineClose  size={22} />:<HiOutlineBars3  size={22}/>}</button>
